...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(ExceptionType::CONVERSION, "lower() takes exactly 1 argument");
  }

  if (args[0]->GetReturnType() != TypeId::VARCHAR) {
    throw Exception(ExceptionType::CONVERSION, "arg must be of type VARCHAR");
  }

  StringExpressionType type;
  if (func_name == "lower") {
    type = StringExpressionType::Lower;
  } else if (func_name == "upper") {
    type = StringExpressionType::Upper;
  } else {
    throw Exception(ExceptionType::CONVERSION, "unknown function name");
  }
